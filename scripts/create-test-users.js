...
async function createTestUsers() {
  ...
  const publicUsers = testUsers.map(u => ({
    ...
    email: u.email,
    full_name: u.email.split('@')[0].replace(/[0-9]/g, ' ').trim().split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '),
  ...
  fs.writeFileSync(
    'test-tokens.json',
    JSON.stringify({ tokens, created_at: new Date().toISOString() }, null, 2)
  );
