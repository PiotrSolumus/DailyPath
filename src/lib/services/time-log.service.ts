...
export async function listTimeLogs(
  ...
  try {
    ...
    let query = supabase.from("time_logs").select("id, task_id, user_id, period");
...
export async function createTimeLog(
  ...
  try {
    ...
    // Insert time log
    const { data: timeLog, error } = await supabase.from("time_logs").insert(insertData).select("id").single();
...
export async function updateTimeLog(
  ...
  try {
    ...
    const { error } = await supabase.from("time_logs").update(updateData).eq("id", timeLogId);
 ...
 */
export async function getTaskLoggedMinutes(supabase: SupabaseClient<Database>, taskId: string): Promise<number> {
  try {
    const { data: logs, error } = await supabase.from("time_logs").select("period").eq("task_id", taskId);
