 ...
 * ```
 */
export function requireAuth(Astro: AstroGlobal, redirectTo?: string): UserMeDTO {
 ...
 * ```
 */
export function requireRole(Astro: AstroGlobal, requiredRole: "admin" | "manager" | "employee"): UserMeDTO {
 ...
 * ```
 */
export function hasRole(Astro: AstroGlobal, role: "admin" | "manager" | "employee"): boolean {
  const user = Astro.locals.user;
