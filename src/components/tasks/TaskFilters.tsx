...
export function TaskFilters() {
  const { filters, setStatus, setPriority, setDepartmentId, setUserId, setIsPrivate, clearFilters } = useTaskFilters();
  ...
  return (
    <section className="space-y-4 rounded-lg border bg-card p-4" aria-labelledby="filters-heading">
      <div className="flex items-center justify-between">
        <h3 id="filters-heading" className="font-medium">
          Filtry
        </h3>
        {hasActiveFilters && (
          <Button variant="ghost" size="sm" onClick={clearFilters} aria-label="Wyczyść wszystkie filtry">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="space-y-2">
          ...
          <Select id="status-filter" value={filters.status ?? ""} onChange={(e) => setStatus(e.target.value || null)}>
